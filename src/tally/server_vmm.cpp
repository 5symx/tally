        ...
        .and_then([&](auto& responsePayload) {
            ...
            if (args->kind == cudaMemcpyHostToDevice) {
                ...
                if(!client_data_all[client_id].finish_init)
                {
                    ...
                    if(stream != client_data_all[client_id].copy_stream)
                    {
                        client_data_all[client_id].copy_stream = stream;
                        TALLY_SPD_LOG("update stream");
                    }
                    // client_data_all[client_id].copy_queue_H2D = true;
...
void TallyServer::handle_cudaStreamSynchronize(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    ...
    iox_server->loan(requestHeader, sizeof(cudaError_t), alignof(cudaError_t))
        .and_then([&](auto& responsePayload) {
            ...
            if(stream == client_data_all[client_id].copy_stream && client_data_all[client_id].finish_init == false)
            {
