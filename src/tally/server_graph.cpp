...
void TallyServer::handle_cudaMalloc(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    ...
    iox_server->loan(requestHeader, sizeof(cudaMallocResponse), alignof(cudaMallocResponse))
        .and_then([&](auto& responsePayload) {
            ...
            if(!finish_init_graph)
            {
                if(client_data_all[client_id].init_stream != nullptr) // launch first kernel
                {
                    TALLY_SPD_LOG("finish graph for init");
                    CHECK_CUDA_ERROR(cudaStreamEndCapture(client_data_all[client_id].init_stream, &graph_init));
                    CHECK_CUDA_ERROR(cudaGraphInstantiate(&graphExec, graph_init, NULL, NULL, 0));
                    CHECK_CUDA_ERROR(cudaGraphLaunch(graphExec, client_data_all[client_id].init_stream));
...
void TallyServer::handle_cudaMemcpyAsync(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    ...
    // if (stream != client_data_all[client_id].default_stream) {
    //     stream = client_data_all[client_id].default_stream;
    // }
        ...
        .and_then([&](auto& responsePayload) {
            ...
            if (args->kind == cudaMemcpyHostToDevice) {
                ...
                if(!finish_init_graph)
                {
                    if(client_data_all[client_id].init_stream == nullptr) // launch first kernel
                    {
                        TALLY_SPD_LOG("init graph capture stream");
                        // cudaEventRecord(client_data_all[client_id].all_h2d_done_event, stream);
                        client_data_all[client_id].init_stream = stream;

                        // wait_until_launch_queue_empty(client_id);

                        CHECK_CUDA_ERROR(cudaStreamBeginCapture(client_data_all[client_id].init_stream, cudaStreamCaptureModeGlobal));
                    }
                    TALLY_SPD_LOG("async capture with init_stream");
                    assert(client_data_all[client_id].init_stream == stream);
                    res->err = cudaMemcpyAsync(args->dst, args->data, args->count, args->kind, stream);
                }
                else{
                    TALLY_SPD_LOG("pure async");
                    res->err = cudaMemcpyAsync(args->dst, args->data, args->count, args->kind, stream);
...
void TallyServer::handle_cudaStreamSynchronize(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    ...
    iox_server->loan(requestHeader, sizeof(cudaError_t), alignof(cudaError_t))
        .and_then([&](auto& responsePayload) {
            ...
            if(!finish_init_graph && client_data_all[client_id].init_stream == stream) // during capture
            {
                TALLY_SPD_LOG("bypass during capture init graph stream")

                *response = cudaSuccess; 
            }
            else
            {
                TALLY_SPD_LOG("pure stream sync")
    ...
    iox_server->loan(requestHeader, sizeof(cudaStreamCreateResponse), alignof(cudaStreamCreateResponse))
        .and_then([&](auto& responsePayload) {
            ...
            if (policy == TALLY_SCHEDULER_POLICY::PRIORITY) {
                ...
                TALLY_SPD_LOG("block new cudaStreamCreate");
