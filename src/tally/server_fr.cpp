...
void TallyServer::start_worker_server(int32_t client_id) {
    ...
    while (!iox::posix::hasTerminationRequested())
    {
        ...
        worker_server->take().and_then([&](auto& requestPayload) {
            ...
            // TALLY_SPD_LOG_VALUE("Processing init request for on worker for client_id: {}", client_pid_of_request);
                            ...
                            bool reuse_flag = false)
{
    ...
    if (response->err == cudaSuccess) {
        ...
        {
            dev_addr_map.push_back(mem_region(response->devPtr, args->size));
            TALLY_SPD_WARN("Allocated new memory without Current ID counter"); // Use spdlog's direct formatting
        }
    ...
    iox_server->loan(requestHeader, sizeof(cudaMallocResponse), alignof(cudaMallocResponse))
        .and_then([&](auto& responsePayload) {
            ...
            if(first_round)
            {
                ...
                {
                    handle_cuda_allocation(response, args, dev_addr_map, init_mem_Size, true);
            ...
            {
                if(!client_data_all[client_id].first_init)
                {
                    // TALLY_SPD_WARN("current rc memory id " + std::to_string(rc_mem_Size));
                    response->devPtr = get_addr_by_init_memory_id(dev_addr_map, rc_mem_Size + 1);
                    if (response->devPtr != nullptr)
                    {
                        ...
                        else // init first model malloc only
                        {
                            ...
                            rc_mem_Size++;
                            TALLY_SPD_WARN("recovery current mr size " + std::to_string(rc_mem_Size));
                        }
        ...
        .and_then([&](auto& responsePayload) {
            ...
            *response = cudaSuccess;
            ...
            if (*response == cudaSuccess) {
                ...
                free_mem_region(dev_addr_map, args->devPtr);
            }
