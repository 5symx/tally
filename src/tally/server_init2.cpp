...
void TallyServer::start_main_server() {
    ...
    while (!iox::posix::hasTerminationRequested())
    {
        ...
        handshake_server.take().and_then([&](auto& requestPayload) {
            ...
            TALLY_SPD_LOG("first round start");
            TALLY_SPD_LOG(client_id);
            ...
            // TallyServer::server->start_worker_server(client_id);
...
void TallyServer::start_worker_server(int32_t client_id) {
    ...
    // CHECK_CUDA_ERROR(cudaEventCreate(&client_meta.all_h2d_done_event));
    ...
    // if (finish_first == false)
    // {
    //     TALLY_SPD_LOG("first round finish");
    //     finish_first = true;
    //     client_data_all[client_id].replay_graph = false;
    // }
...
void TallyServer::handle_cudaLaunchKernel(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    ...
    // if (!client_data_all[client_id].finish_init)
    // {
    //     client_data_all[client_id].finish_init = true;
    //     cudaStreamSynchronize(client_data_all[client_id].copy_stream); // stream to update
    //     // cudaStreamWaitEvent(client_data_all[client_id].copy_stream, client_data_all[client_id].all_h2d_done_event, 0);
    // }
...
void TallyServer::handle_cudaMalloc(void *__args, iox::popo::UntypedServer *iox_server, const void* const requestPayload)
{
    ...
    iox_server->loan(requestHeader, sizeof(cudaMallocResponse), alignof(cudaMallocResponse))
        .and_then([&](auto& responsePayload) {
            ...
            if(!finish_first) // first initilization
            {
                TALLY_SPD_LOG("original cudaMalloc");
                if(client_data_all[client_id].init_stream != nullptr && finish_init_graph == false) // launch first kernel
                {
                    TALLY_SPD_LOG("finish graph for init");
                    cudaStreamEndCapture(client_data_all[client_id].init_stream, &graph_init);
                    cudaGraphInstantiate(&graphExec, graph_init, NULL, NULL, 0);
                    cudaGraphLaunch(graphExec, client_data_all[client_id].init_stream);
                    finish_init_graph = true;
                    cudaStreamSynchronize(client_data_all[client_id].init_stream);
                }
                response->err = cudaMalloc(&(response->devPtr), args->size);
            }
            else
            {
                if(client_data_all[client_id].replay_graph == false && client_data_all[client_id].replay_stream != nullptr)
                {
                    cudaStreamSynchronize(client_data_all[client_id].replay_stream);
                    TALLY_SPD_LOG("reprocess graph for initilization");
                    cudaGraphLaunch(graphExec, client_data_all[client_id].replay_stream);
                    client_data_all[client_id].replay_graph = true;
            ...
            // if(client_data_all[client_id].init_stream != nullptr && client_data_all[client_id].finish_init_graph == false) // launch first kernel
            // {
            //     TALLY_SPD_LOG("finish graph for init");
            //     cudaStreamEndCapture(client_data_all[client_id].init_stream, &client_data_all[client_id].graph_init);
            //     client_data_all[client_id].finish_init_graph = true;
            //     cudaStreamSynchronize(client_data_all[client_id].init_stream);
            // }
    ...
    iox_server->loan(requestHeader, res_size, alignof(cudaMemcpyAsyncResponse))
        .and_then([&](auto& responsePayload) {
            ...
            if (args->kind == cudaMemcpyHostToDevice) {
                if(!finish_first) // first initilization
                {
                    TALLY_SPD_LOG("original cudaMemcpyAsync");
                    if(client_data_all[client_id].init_stream == nullptr) // launch first kernel
                    {
                        TALLY_SPD_LOG("init graph stream");
                        // cudaEventRecord(client_data_all[client_id].all_h2d_done_event, stream);
                        client_data_all[client_id].init_stream = stream;
                        
                        cudaStreamBeginCapture(client_data_all[client_id].init_stream, cudaStreamCaptureModeGlobal);
                    }

                    assert(client_data_all[client_id].init_stream == stream);
                    res->err = cudaMemcpyAsync(args->dst, args->data, args->count, args->kind, stream);
                }
                else
                {
                    // update stream
                    if(client_data_all[client_id].replay_stream == nullptr) // launch first kernel
                    {
                        TALLY_SPD_LOG("init current copy stream");
                        // cudaEventRecord(client_data_all[client_id].all_h2d_done_event, stream);
                        client_data_all[client_id].replay_stream = stream;
                    }

                    assert(client_data_all[client_id].replay_stream == stream);
                    if(client_data_all[client_id].replay_graph == false)
                    {
                        TALLY_SPD_LOG("bypass graph for cudaMemcpyAsync");
                        res->err = cudaSuccess;
                    }
                    else
                    {
    ...
    iox_server->loan(requestHeader, sizeof(cudaError_t), alignof(cudaError_t))
        .and_then([&](auto& responsePayload) {
            ...
            if(!finish_first) // first initilization
            {
                ...
                if(stream == client_data_all[client_id].init_stream && finish_init_graph == false)
                {
                    TALLY_SPD_LOG("bypass cudaStreamSynchronize for first round");
                    *response = cudaSuccess; // by pass sync
                }
                else
                {
                    TALLY_SPD_LOG("original cudaStreamSynchronize");
            ...
            {
                if(client_data_all[client_id].replay_graph)
                {
                    *response = cudaStreamSynchronize(
                        stream
                    );
                ...
                {
                    TALLY_SPD_LOG("bypass graph for cudaStreamSynchronize");
                    *response = cudaSuccess;
                }
            ...
            // if(stream == client_data_all[client_id].init_stream && client_data_all[client_id].finish_init_graph == false)
            // {
