...
# Example usagell
log_path = "./naive-con6-27b.log" #"./txt-naive-27b.log" # ./txt-naive-27b.log
...
for idx, entry in enumerate(entries):
    ...
    try:
        decode_time = float(entry.get('decode_time', 0))
    except (ValueError, TypeError):
        continue
    
    group0.append(decode_time)
    if len(group0) == 30:
        break
...
# Example usagell
log_path = "./mps-con6-27b.log" #"./txt-mps-27b.log" # ./txt-naive-27b.log
entries = extract_split_log_entries(log_path)
...
# Print result rounded to two decimals
print(f"Group naive's decode_time is {increase_percent:.2f}% higher than Group mps's.")



# Example usage
log_path = "./gm-con6-27b.log"#"./txt-fr3-27b.log"
entries = extract_split_log_entries(log_path)
...
for idx, entry in enumerate(entries):
    ...
    if not group1:
        group1.append(decode_time)
    elif prev_empty:
        group1.append(decode_time)
        prev_empty = False
    else:
        group2.append(decode_time)
    if len(group1) == 50:
        break
...
# Print result rounded to two decimals
print(f"Group naive's decode_time is {increase_percent:.2f}% higher than Group 2's.")
...
# Print result rounded to two decimals
print(f"Group mps's decode_time is {increase_percent:.2f}% higher than Group 2's.")